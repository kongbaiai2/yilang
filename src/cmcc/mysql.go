...
// 插入db，重复则替换
func initInfoToDb() {
	CmccDomainAdd("zhimage.guangdongyunchen.com", "cmcc", "zhuhaiyidong")
...
func writeBwToDb(resultStruct []ResponseStatistic) error {
	var sum float64
	for _, v := range resultStruct {
		// log.Println(v.Domain, len(v.Data))
		for _, vv := range v.Data {
			// log.Println("-------------", vv.Time, len(vv.Provinces))
			sum = 0
			...
			for _, data := range vv.Provinces {
				// log.Println(vvv.Value)
				err := StatisticBwReplace(vv.Time, data.Area, data.Value, unixTime)
				if err != nil {
					return err
				}
				if data.Area == TOTAL {
			...
			// 数据为0直接告警
			if sum == 0 {
				warnerChan <- NewAlarmDb("珠海移动cdn数据API告警", "数据为零: ", v.Domain, "mail", 1)
			}
...
func StatisticBwReplace(time, provinces string, value float64, unix_time int64) error {
...
func StatisticBwSelectProvinces(total string, num int) ([]StatisticBw, error) {
	// ins := &StatisticBw{Time: time, Provinces: provinces, Value: value, UnixTime: unix_time}
	var resq []StatisticBw
	query := db.Order("updated_at")
	if total != "" {
		query = db.Where("Provinces = ?", total).Order("unix_time desc").Limit(num)
	}
